...
########################## PART 4 ##########################
"""
Keeping the viscous force, we can add a pulsation force Acos(wt), with different values of w.
The equation of motion of the pendulum with the forcing pulsation is
    theta''(t) = -(g/L) * sin(theta(t)) - beta * theta'(t) + A * cos(wt)
which can be decomposed in
    omega'(t) = -(g/L) * sin(theta(t)) - beta * omega(t) + A cos(wt)
    theta'(t) = omega(t)
and can again be integrated using the runge-kutta algorithm.
The resulting trajectories are shown together with the ones of the pendulum without friction nor pulsation.
"""
...
def derivatives_pendulum_with_friction_and_force(state, t):
    """
...
state_forced = np.array([[theta_0,omega_0]]) #create arrays containing the initial angle/velocity/location of the pendulum
x_y_forced = pendulum_location(theta_0,L)
...
for index,time in enumerate(t[1:]): #update the pendulum angle/velocity/location using the runge kutta integration
    state_new_forced = runge_kutta(state_forced[index,:],time,dt,derivatives_pendulum_with_friction_and_force)
    x_y_new_forced = pendulum_location(state_new_forced[0],L)
    ...
    state_forced = np.vstack((state_forced,state_new_forced))
    x_y_forced = np.vstack((x_y_forced,x_y_new_forced))
...
def execute_part4():
    ...
    anim_theta_forced,         = ax[0,0].plot([],[],'b-')
    ...
    anim_omega_forced,         = ax[0,1].plot([],[],'r-')
    ...
    def init():
        anim_theta_forced.set_data([], [])
        anim_theta_simple.set_data([], [])
        anim_omega_forced.set_data([], [])
    ...
    def animate(i, state,state_sa,x_y,x_y_sa):
        ...
        anim_theta_forced.set_data(t[0:i],state_forced[0:i,0])
        anim_theta_simple.set_data(t[0:i],state_simple[0:i,0])
        anim_omega_forced.set_data(t[0:i],state_forced[0:i,1])
        anim_omega_simple.set_data(t[0:i],state_simple[0:i,1])
        anim_pendulum.set_data([0,x_y_forced[i,0]],[0,x_y_forced[i,1]])
        anim_trajectory.set_data(x_y_forced[0:i,0],x_y_forced[0:i,1])
        anim_phase.set_data(state_forced[0:i,0],state_forced[0:i,1])
    ...
    #Call the animator
    anim = animation.FuncAnimation(fig, animate, init_func=init, fargs=(state_forced,state_simple,x_y_forced,x_y_simple), interval=10)
    return anim    
